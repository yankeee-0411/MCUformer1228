...
class EvolutionSearcher(object):
    def __init__(
        ...
        device,
        model,
        model_without_ddp: Vision_TransformerSuper,
        choices,
        val_loader,
        test_loader,
        output_dir,
    ...
    def is_legal(self, cand):
        ...
        sampled_config = {}
        # sampled_config["patch_sizes"] = args.patch_size
        ...
        # ADD
        peak_memory = self.model_without_ddp.get_maximum_memory(sampled_config)
        ...
        # ADD
        if info["peak_memory"] > self.args.memory_limit:
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            for i, cand in enumerate(self.keep_top_k[50]):
                ...
                print(
                    "No.{} {} Top-1 val acc = {}, Top-1 test acc = {}, params = {}, peak_memory = {}".format(
                        i + 1,
                        cand,
                        self.vis_dict[cand]["acc"],
                        self.vis_dict[cand]["test_acc"],
                        self.vis_dict[cand]["params"],
                        self.vis_dict[cand]["peak_memory"],
                    )
                )
...
def get_args_parser():
    ...
    parser.add_argument("--rank_ratio", default=0.7, type=float)
    ...
    parser.add_argument(
        "--memory_limit", type=int, default=320000, help="memory limit in bytes"
    )
...
def main(args):
    ...
    dataset_test, _ = build_dataset(
        is_train=False, args=args, folder_name="val")
    ...
    model = Vision_TransformerSuper(
        ...
        patch_size=int(cfg.EVOLUTION.PATCH_SIZE),
        ...
        rank_ratio_attn=cfg.EVOLUTION.RANK_RATIO_ATTN,
        rank_ratio_mlp=cfg.EVOLUTION.RANK_RATIO_MLP,
    )
    ...
    # choices = {
    #     "patch_sizes": cfg.EVOLUTION.PATCH_SIZE,
    ...
    t = time.time()
    args.patch_size = cfg.EVOLUTION.PATCH_SIZE
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "AutoFormer evolution search", parents=[get_args_parser()]
    )
